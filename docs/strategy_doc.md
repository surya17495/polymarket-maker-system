...
### Next-concrete-blocks-queue (post-session copiétés)

1. Wire `/trades` ground-truth fetcher into the lab: after each `_register_placement`, query `lib/trades_truth.py::fetch_all_trades(condition_id, asset_id, t_arrival_delta_ms)` for the asset's trade history within the quote-timeout window. Matches BUY/SELL trades at our quote price against our simulated placement side; authoritative fill emitted only on trade-side agreement. Should reduce the 40% noise floor for the AS regressor's ground truth.
2. Debug S2..S6 underperformance: regime's `halt_before_hours=1` default kills soon-to-resolve esports markets too aggressively (every >5-min-to-resolve events ends up HALTED). Tighten to halt_before_hours=0.25 for first-half window only; antithrash's `Δmid<0.5c and Δsize<10%` filter is too aggressive for slow markets — widen to `Δmid<0.10c or size unchanged`. The cumulative-layering strategy cataloque is good for attribution-per-feature but means bugs at the base propagate up.
3. Phase 1B walk-forward: `--walk-forward --window-minutes 30` runs the lab on each 30-min test window using the prior window's strategy (or parameter sweep result) as the selected-trade plan. The lab already supports this; missing the per-window train-phase parameter-optimizer.
4. Cross-model Codex review: extend the lab to support longer `raw_events` snapshots from the live capture (which is still in 48h-rotating mode as of this write). /trades integration is the unblock-able prerequisite before any Phase 1B statistical gate attempt.
5. After validation, the `s1_poly_quoting` strategy becomes the default strategy for `main_paper.py::phase_1a` (replacing `BBTickStrategy`); the lab then becomes the live-paper-trader dry-run selector before each Phase 2A-Operational deployment.
